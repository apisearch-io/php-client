...
class ItemTest extends PHPUnit_Framework_TestCase
{
     ...
     */
    public function testSearchableMetadata()
    {
        $searchableMetadata = [
            'a' => '1',
            'b' => 2,
        ...
        $this->assertSame($searchableMetadata, $item->getSearchableMetadata());
        $this->assertSame($searchableMetadata, $item->getAllMetadata());
        $this->assertSame('1', $item->get('a'));
        $this->assertSame(2, $item->get('b'));

        $item->setSearchableMetadata(['c' => true, 'd' => ['e' => 'hola']]);
        $this->assertEquals('hola', $item->getSearchableMetadata()['d']['e']);
        $this->assertEquals('hola', $item->getAllMetadata()['d']['e']);
        $this->assertCount(2, $item->getAllMetadata());

        $item->addSearchableMetadata('z', 10);
        $this->assertCount(3, $item->getAllMetadata());
        $this->assertEquals(10, $item->getSearchableMetadata()['z']);
    }

    /**
     * Test exactMatchingMetadata.
     */
    public function testExactMatchingMetadata()
    {
        ...
        $item = Item::createFromArray([
            ...
            'exact_matching_metadata' => $exactMatchingMetadata,
        ]);

        $this->assertSame($exactMatchingMetadata, $item->getExactMatchingMetadata());
        $this->assertSame($exactMatchingMetadata, $item->getAllMetadata());
        $this->assertSame('1', $item->get('a'));
        $this->assertSame(2, $item->get('b'));

        $item->setExactMatchingMetadata(['c' => true, 'd' => ['e' => 'hola']]);
        $this->assertEquals('hola', $item->getExactMatchingMetadata()['d']['e']);
        $this->assertEquals('hola', $item->getAllMetadata()['d']['e']);
        $this->assertCount(2, $item->getAllMetadata());

        $item->addExactMatchingMetadata('z', 10);
        $this->assertCount(3, $item->getAllMetadata());
        $this->assertEquals(10, $item->getExactMatchingMetadata()['z']);
    }
